...
def probability_exactly(population_size, success_count, sample_size, hits):
    ...
    if hits > success_count or hits > sample_size or sample_size - hits > population_size - success_count:
        return 0.0
    return comb(success_count, hits) * comb(population_size - success_count, sample_size - hits) / comb(population_size, sample_size)
...
def probability_at_least(population_size, success_count, sample_size, minimum_hits):
    ...
    return sum(probability_exactly(population_size, success_count, sample_size, hits) for hits in range(minimum_hits, max_hits + 1))
